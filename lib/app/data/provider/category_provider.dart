...
class CategoriesProvider {
  ...
  Stream<List<CategoryModel>> getAllCategories({required String userUid}) {
    return _firebaseFirestore.doc(userUid).collection('category').orderBy('catName').snapshots().map((query) {
  ...
  Future<void> addCategory({
    ...
    DocumentReference documentReference = _firebaseFirestore.doc(userUid).collection('category').doc();
  ...
  Future<void> updateCategory({
    ...
    DocumentReference documentReference = _firebaseFirestore.doc(userUid).collection('category').doc(catUid);

    Map<String, dynamic> data = <String, dynamic>{'catName': catName, 'catType': catType, 'catDescription': catDescription};
